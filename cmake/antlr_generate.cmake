...
file(TO_CMAKE_PATH "${CMAKE_SOURCE_DIR}/grammar/${GRAMMAR_PARSER_NAME}.g4" GRAMMAR_PARSER_PATH)
set(GRAMMAR_PARSER_PATH "${GRAMMAR_PARSER_PATH}" CACHE FILEPATH "Path to the grammar parser file.")

file(TO_CMAKE_PATH "${CMAKE_SOURCE_DIR}/grammar/${GRAMMAR_LEXER_NAME}.g4" GRAMMAR_LEXER_PATH)
set(GRAMMAR_LEXER_PATH "${GRAMMAR_LEXER_PATH}" CACHE FILEPATH "Path to the grammar lexer file.")
...
if(DEFINED ANTLR_NAMESPACE)
  set(
    ANTLR_COMMAND
      ...
      -o "${ANTLR_GEN_DIR}" -package "${ANTLR_NAMESPACE}" "${GRAMMAR_PARSER_PATH}" "${GRAMMAR_LEXER_PATH}"
  )
else()
  set(
    ANTLR_COMMAND
      "${Java_JAVA_EXECUTABLE}" -jar "${ANTLR_JAR}" -Werror -Dlanguage=Cpp -listener -visitor
      -o "${ANTLR_GEN_DIR}" "${GRAMMAR_PARSER_PATH}" "${GRAMMAR_LEXER_PATH}"
  )
endif()
...
set(
  ANTLR_GEN_SRC
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}BaseListener.cpp"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}BaseVisitor.cpp"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Lexer.cpp"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Listener.cpp"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Parser.cpp"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Visitor.cpp"
)

set(
  ANTLR_GEN_HEADERS
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}BaseListener.h"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}BaseVisitor.h"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Lexer.h"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Listener.h"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Parser.h"
    "${ANTLR_GEN_DIR}/${GRAMMAR_PARSER_NAME}Visitor.h"
)

# Create the command to generate the sources.
add_custom_command(
  OUTPUT ${ANTLR_GEN_SRC} ${ANTLR_GEN_HEADERS}
  DEPENDS antlr "${GRAMMAR_PARSER_PATH}" "${GRAMMAR_LEXER_PATH}"
  COMMAND
